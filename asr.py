...
class Settings(BaseModel):
    ...
    def set_lot(self, lot_id: str, tag: str, extend_lot_id: Optional[str] = None, note: Optional[str] = None):
...
def _match_lot_by_tag_keyword(description: str) -> Optional[str]:
    """
    Шаг 5.5 (улучшенный): ищет тег для заказа двумя способами.

    5.5а: ищет @тег прямо в тексте описания заказа (самый надёжный способ).
          Если в описании заказа написано @pz — сразу возвращает лот с тегом pz.
    ...
    at_tag = _parse_at_tag_from_text(description)
    if at_tag:
        lot_id = SETTINGS.find_lot_id_by_tag(at_tag)
        if lot_id:
            logger.info(
                f"[ASRplus] _match_lot_by_tag_keyword: найден @тег '{at_tag}' "
...
def _find_lot_id_for_order(c, event) -> Optional[str]:
    ...
    logger.warning(f"[ASRplus] #{order_id}: лот не определён по шагам 1-5.5 — заказ скипается (не наш товар)")
    return None
    ...
    def _do_process():
        ...
        try:
            ...
            if not lot_id:
                ...
                # Заказ не наш: помечаем как игнорируемый — ни аккаунт, ни возврат, ни сообщение.
            ...
            with _data_lock:
                ...
                if existing and order_id not in ORDERS:
                    ...
                    if acc and acc.rental_end:
                        ...
                        _cancel_extend_lot_timer(lot_id)
                        # Отключаем лот-продление обратно после покупки
                        threading.Thread(
                            target=lambda: _toggle_single_lot(c, lot_id, False),
                            daemon=True
                        ).start()
...
def process_message(c, event):
    ...
    if is_code:
        ...
        if extend_lot_id:
            ...
            # Включаем лот-продление
            threading.Thread(
                target=lambda: _toggle_single_lot(c, extend_lot_id, True),
                daemon=True
            ).start()
            # Запускаем таймер: если через 5 минут не купят — лот отключится
...
def init(card: Cardinal):
    ...
    def open_about(c):
        ...
        text = (
            ...
            f"∟ Разработчик: <a href=\"https://t.me/Dzhants\">@Dzhants</a>\n\n"
            f"📢 Канал: <a href=\"https://t.me/Dzhants\">t.me/Dzhants</a>"
        )
        kb = K(row_width=1)
    ...
    def open_acc_by_tag(c):
        ...
        if not selected_tag or selected_tag == '0':
            ...
            for tag in sorted(tags):
                ...
                kb.add(B(f'🏷 {tag}  ({total_tag} акк / {free} 🟢)', None, f'{CBT.ACC_BY_TAG}:{tag}'))
            kb.add(B('⬅️ Назад', None, CBT.ACC_MENU))
            ...
            for tag in sorted(by_tag.keys()):
                for lid in by_tag[tag]:
                    ...
                    if lc:
                        ...
                        kb.add(B(f"[{lc.tag}] {display}  ·  {free} 🟢", None, f"{CBT.LOT_DETAIL}:{lid}"))
    ...
    def open_lot_detail(c):
        ...
        text = (f"<b>🔗 Лот #{lid}</b>\n\n∟ Тег: <code>{lc.tag}</code>\n"
                ...
                f"∟ Статус на FunPay: {active_str}\n∟ Ссылка: {lot_url}{note_str}{extend_info}")
        kb = K(row_width=2)
    ...
    def _h_extend_lot_id(m):
        ...
        _invalidate_lots_cache()
        send(m.chat.id,
             f"✅ Лот-продление <code>#{extend_id}</code> привязан к лоту <code>#{main_lot_id}</code>",
             _back_kb(f"{CBT.LOT_DETAIL}:{main_lot_id}"))
    ...
    tg.cbq_handler(open_about, lambda c: c.data == CBT.ABOUT)
    ...
    try:
        for uid in tg.authorized_users:
            bot.send_message(
                uid,
                "✅ <b>ASR+</b> успешно установлен!\n\n"
                "Плагин создан: <b>@Dzhants</b>\n"
                "📢 Подпишись на канал: <a href=\"https://t.me/Dzhants\">t.me/Dzhants</a>",
                parse_mode="HTML",
